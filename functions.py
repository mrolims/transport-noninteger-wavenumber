...
def format_number(x, decimal_places=5):
    if abs(x) < 10**-decimal_places:  # effectively zero
        x = 0.0
...
def generate_values_around_integer(n):
    ...
    below_n = (
        np.arange(n - 0.05, n, 0.01).tolist() + np.arange(n - 0.005, n, 0.001).tolist()
    )

    # Generate values above n
    above_n = (
        [n]
        + np.arange(n + 0.001, n + 0.006, 0.001).tolist()
        + np.arange(n + 0.01, n + 0.06, 0.01).tolist()
    )
