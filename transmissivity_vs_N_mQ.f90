program main
    ...
    real(wp) :: golden_ratio = (1.0_wp - sqrt(5.0_wp)) / 2.0_wp
    ! --- Variables --- !
    integer(8) :: i, j, k, N
    integer :: num_ic, num_remaining
    real(wp), dimension(:), allocatable :: x
    real(wp), dimension(:), allocatable :: y
    real(wp), dimension(:), allocatable :: x_temp
    real(wp), dimension(:), allocatable :: y_temp
    ...
    integer, dimension(:), allocatable :: seed ! seed array
    ...
    m = [golden_ratio, -golden_ratio, sqrt(2.0_wp), -sqrt(2.0_wp), sqrt(3.0_wp), -sqrt(3.0_wp), sqrt(5.0_wp), -sqrt(5.0_wp), sqrt(6.0_wp), -sqrt(6.0_wp), sqrt(7.0_wp), -sqrt(7.0_wp), sqrt(8.0_wp), -sqrt(8.0_wp), sqrt(10.0_wp), -sqrt(10.0_wp), sqrt(11.0_wp), -sqrt(11.0_wp), sqrt(12.0_wp), -sqrt(12.0_wp), sqrt(13.0_wp), -sqrt(13.0_wp), sqrt(14.0_wp), -sqrt(14.0_wp)]

    do k = 1, size(m)

        write(unit=datafile, fmt=1000) trim(path), m(k), y0, num_ic, N, esc_y
        ...
        do j = 1, num_ic
            do while (.true.)
                call final_SALI(x(j), y(j), a, b, c, m(k), 1000, sali)
                if (sali < 1e-12) then
                    exit
                else
        ...
        T = 0.0_wp
        do i = 1, N
            call mapping_ensemble(x, y, a, b, c, m(k))
